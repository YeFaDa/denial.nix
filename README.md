...
`denial-update-check` reports every input belonging to the same immutable
Denial release tag. Source checking is enabled by default; it reads the release
`SOURCE_LOCK.json`, the matching Flutter fork's `bin/internal` paths, and the
fixed Dart revision from Flutter `DEPS`. It never follows `main`, `master`, a
moving branch, or an unpinned `latest` URL.
...
the exact update actions for `Cargo.lock`, `gclient-deps.json`, both Pub locks,
and `flake.lock`. Source dependency hashes are generated by `gclient2nix`; they
are not duplicated as manual fields in this repository.
...
1. If `has_update` is `1`, update `pkgs/version.nix`,
   `pkgs/prebuilt-hashes.nix`, and `pkgs/denial/Cargo.lock` using the reported
   immutable tag and archive hashes.
2. Regenerate `pkgs/denial-flutter-engine/gclient-deps.json` from the reported
   release `SOURCE_LOCK.json` with `gclient2nix`; do not copy source hashes by
   hand.
3. Update `nixpkgs-dart` to a locked revision whose `dart-bin` matches the new
   Dart revision, then regenerate the Flutter tools and shell Pub locks with
   the matching toolchain.
4. Update the two `bin/internal` paths in `revisions.nix`, run `nix flake
   lock --update-input nixpkgs-dart`, and verify both `denial` and
   `denial-source`.
...
5. Run `nix flake check --all-systems --no-build --no-write-lock-file`.
6. Build the default Rust-source package with prebuilt Flutter artifacts:
   `nix build .#denial --no-link --fallback`.
7. Build the complete source package when source pins changed:
   `NIX_BUILD_CORES=16 nix build .#denial-source --no-link --fallback`.
8. Verify `deniald --version`, `libflutter_engine.so`, `libapp.so`, and
   `flutter_assets`; review and commit all pins together.

The daily GitHub workflow runs the same complete report, uploads it as the
`denial-update-report` artifact, and fails with a clear maintenance signal when
any release or source pin differs. It does not edit or commit the main branch.
